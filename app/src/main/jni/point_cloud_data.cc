...
void PointCloudData::UpdatePointCloud(const TangoXYZij* point_cloud) {
  ...
  prev_frame_timestamp_ = point_cloud->timestamp;
    ...
    std::string PointCloudData::GetStringXYZij() {

      std::ostringstream oss;
        oss << "timestamp: " << cur_frame_timstamp_ << "\n";
      if (!vertices_.empty()) {
        // Convert all but the last element to avoid a trailing ","
        //for (i=0,i< (sizeof(vertices_)/sizeof(vertices_[0]))/3,i++){
        //    std::copy(std::copy())

                                        // just for presentation purposes
        std::copy(vertices_.begin(), vertices_.begin() + 2,//vertices_.end() - 1,
                  std::ostream_iterator<float>(oss, ","));

        // Now add the last element with no delimiter
        oss << vertices_.back() << "...etc";
      }
      return oss.str();
    }

    //std::<float> PointCloudData::OutputXYZij(){
